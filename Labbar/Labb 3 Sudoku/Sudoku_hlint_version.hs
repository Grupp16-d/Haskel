...
isSudoku sud = numRows sud && numElem sud && valueElem sud
  where
    numRows   sud = length (rows sud) == 9
    numElem   sud = and [length r == 9| r <- rows sud]
    valueElem sud = and [and [isNothing x || (x > Just 0 && x < Just 10)
                                               | x <- r] | r <- rows sud]
-- A3
...
isOkayBlock b = length (nubBy (\x y -> x == y && isJust x) b) == 9
